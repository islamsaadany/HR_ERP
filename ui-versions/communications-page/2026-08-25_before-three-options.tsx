  ...
  return (
    <div>
      ...
      <p className="mt-1 max-w-[70ch] text-muted">
        Announcements to the people you choose. Each person receives their own copy, branded with
        their own business unit — nobody sees anybody else&rsquo;s address.
      </p>

      <div className="mt-2 flex flex-wrap items-center gap-4 text-sm">
        <Link href="/admin/communications/queue" className="text-muted hover:text-ink">
          Messages waiting to send →
        </Link>
        <Link href="/admin/communications/settings" className="text-muted hover:text-ink">
          Setup: sender and delivery →
        </Link>
      </div>
      ...
      {messages.length === 0 ? (
        <p className="mt-6 rounded-xl border border-line bg-surface p-5 text-sm text-muted">
          Nothing yet. An announcement is written when there is something to say — nothing here is
