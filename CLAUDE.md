...
- **A broadcast is not a transactional email and must not inherit its stance** — the claim and
  ...
  confirmation.
- **Email is limited to FIVE workflows (specs 020 + 035 + 037 + 039 + 040)** — the original "no emails, ever (v1)" rule was reversed for the benefit-claim workflow (approved 2026-08-10), widened to the holiday/vacation workflow (approved 2026-08-19), widened twice more on 2026-08-24: to **Team Communications** (announcements to a chosen audience, plus personal congratulations for birthdays and joining anniversaries) and to the **payback workflow** (requested by the CEO: submitted→Finance, declined/paid→the requester), and on 2026-09-08 to the **time-off request cycle** (CEO: *"someone requested a vacation and I didn't get the email"* — new request→the approver(s) whose queue it landed in, decision→the requester; spec 035 FR-013). All send via **Resend**, **env-gated** (`RESEND_API_KEY`/`EMAIL_FROM`), configurable + master-toggleable at **Admin → Notifications**. The four transactional workflows are **fire-and-forget** (never block a state change); the broadcast one reports per recipient (see the rule above). **An email's recipients are derived through the same rule as the screen that shows the thing** — the leave-request email asks `leaveApproversFor`, the inverse of `pendingApprovalWhere`, so the person whose queue just grew is exactly the person told. **No scheduled process may email an EMPLOYEE** — that half is untouched: cron prepares drafts and nudges operators, a human sends. Still **no** invitations, marketing, external recipients, or scheduling a send for later. **Petty cash itself sends nothing** — the custodian and Finance are both looking at a live screen, and a notification for a ledger nobody is waiting on is email surface bought for no one's benefit.
