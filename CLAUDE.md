...
- **A GUARD MUST START FROM THE THING IT PROTECTS, not from the thing being checked (2026-08-26)** — the rule that an edit may not change an already-released incentive figure was written the obvious way: walk the recomputed report's lines and check the ones marked released. It had a hole precisely where it mattered most. Deleting a person, or deleting the assignment their fee came from, produces **no line at all** — so there was nothing to check and the edit sailed through, while their money had already gone and the report now said they earned nothing. Rewritten to start from the released payments and look up what the edit would produce, treating "absent from the report" as the 0.00 it is. Ask what the guard is protecting (the payments) and iterate THAT, never the collection that happens to be in front of you — a check that can be skipped by making its subject disappear is not a check.
- **A file the type checker never sees is not code, it is a wish (2026-08-26)** — `tsconfig`'s `include` is `**/*.ts`, which does **not** match `.mts`. All 21 `verify-*.mts` scripts had therefore never been type-checked, and turning the glob on surfaced five errors — **two of them breakage introduced earlier the same day, in a script whose entire job is to catch breakage**. They still "passed" because they never got far enough to fail. Extensions are not a detail of a build config: check what the config actually matches, the same way an enum's order or a framework's rendered metadata has to be read from the running system rather than inferred from the source that looks right.
- **A NATIVE date input cannot carry a house date format, and a date-only string must never be parsed to print it (2026-08-25)** — the review-table editor reached for `<input type="date">`, which is what everything else in the app uses for date *plumbing*. A native picker draws itself in the **browser's own UI language**: measured in a real browser, 1 March 2021 rendered `03/01/2021` under en-GB, ar-EG **and** en-US, so the operator would type dd/mm/yyyy into a field labelled the American way round. Where a date is genuinely being *entered*, the field is typed text stating its format, parsed and refused on the server. **And where a person checks their own typing, spell the month**: the incentive module reads and writes `14-Jul 2026` (`dd-mmm yyyy`) — the CEO's reason was *"just to make sure that the entries are correct"*, and two numbers either side of a slash can never show that, however they are ordered. It is a **deliberate exception** to the platform's dd/mm/yyyy (`formatDate`), scoped to the one screen where compensation dates are typed in off a spreadsheet; do not "fix" it back into consistency. One module (`src/lib/incentive/dates.ts`) both formats and parses, because a format and a parser in two files is how display, cell, stored value and CSV quietly stop agreeing. Two more from the same hour: printing a stored date-only string means **reordering the string**, never `new Date(iso).toLocaleDateString(…)` — an ISO date-only string is UTC midnight and a viewer behind UTC gets the previous day (`formatDateISO`); and `new Date("01/03/2021")` is **American**, so any importer that leans on it silently reads an operator's 1 March as 3 January — no warning, no wrong-looking output, just an assignment closing in the wrong quarter. Day-first, everywhere a person's sheet is read.
...
- **A `"use server"` file may export NOTHING but async functions — and one stray export breaks
  EVERY action on the page (2026-08-25)** — `access-actions.ts` also exported an `ACCESS_FIELDS`
  array. Next validates a page's *whole* server-action entry the first time any action on that page
  is called, so the array made every action on the course page throw before it ran (*A "use server"
  file can only export async functions, found object*): the visibility switch, adding people,
  removing a choice and Publish all POST to the page URL and all came back as a bare 500 with no
  message, so one fault presented as three unrelated ones and none of them could report anything —
  the failure is in the action entry, above any code that could catch it. **Nothing catches this**:
  it is not a type error and `next build` compiles it happily, because the check is generated code
  that runs when the action module loads. So the rule is kept by hand — a value, an array, a type's
  runtime twin, anything that is not an async function, goes in a plain module and the action file
  imports it (`src/lib/learning/access-fields.ts`). Related, and the reason it was found at all: a
  digest-only "error occurred in the Server Components render" is worth **driving in a real browser
  against a real Postgres** rather than reading code for — the page's own GET was fine, and only the
  POST failed.
- **A hover `transform` makes a card its own stacking context, so a menu inside it cannot escape
  (2026-08-25)** — `.ff-card:hover` lifts a row by 2px, and a transform creates a stacking context:
  the ⋯ menu's `z-30` was trapped inside its own card and the NEXT row in the list painted straight
  over it. Since the pointer is by definition on the card while its menu is open, this was every
...
- **A hand-written list is a checklist somebody has to remember, so a new module isn't released until it's ON one (2026-08-26)** — Admin → Modules renders `MODULES` (`src/lib/modules.ts`), which does not discover routes; Payback shipped with a nav entry, a page and a Finance queue but no switch, and nothing anywhere said so — not the type check, not the build, not the page (a list of eight looks complete). Two rules: adding an employee-facing module means adding it to that array **and** calling `requireModuleEnabled` on its page *and* its write actions in the same commit — a nav entry that vanishes while the address still works is a nav filter, not a switch; and where a switch covers only part of a module, say which part **in the list itself** — `payback` closes the employee door and deliberately leaves Finance's queue open, because money already submitted must still be payable, and the next person to read the array is the one who needs to know that.
- **An upload's real ceiling is the SMALLEST limit anywhere in the path — and the layers above the form don't announce themselves (2026-09-01)** — every upload here travels through a server action, and Next.js refuses action request bodies over **1MB by default**: smaller than every cap the forms advertise (10MB claim/payback proofs, 25MB handbook/knowledge/learning files). Any receipt photo over 1MB therefore died BEFORE the action ran — no inline message, just the raw "Application error" page — which is exactly how the whole team's benefit claims failed (reported and reproduced with a 3MB file in a real browser, 2026-09-01; a 300KB file sailed through, which is why it ever looked shipped). Three rules: when a form accepts a file, check the limit of **every layer the request crosses** (Next's `serverActions.bodySizeLimit`, now 26MB in `next.config.mjs`, sized to the largest advertised upload; the host's own request cap; the handler's check) — the handler's own size check is unreachable for a body the framework refuses to read; mirror the size cap **client-side before dispatching**, so an oversized file is refused in words next to the button; and **catch the dispatch itself**, because a request that dies in transport (host cap, dropped connection) rejects the action promise and an uncaught rejection paints the error page over the whole app. And where the file is a PHOTO, don't make the person manage its size at all: a receipt only has to be legible, so a large image is scaled down and re-encoded in the browser before dispatch (`src/lib/benefits/proof-image.ts` — a 35MB worst-case photo crossed the wire at 1.4MB, under Vercel's ~4.5MB platform request cap), best-effort only (non-images and undecodable files pass through untouched to the ordinary checks, so shrinking may never be the reason a claim fails). Residual, stated not hidden: a NON-image over the platform cap (a huge PDF, or a HEIC the browser can't decode) can still be refused — with an inline sentence, not a crash; if that ever bites, the fix is uploading from the browser straight to file storage.
- **When a step MOVES, the screen it left behind has to say where it went — and a structural guarantee removed by a redesign must be rebuilt in words (2026-09-08)** — reported as "the finance is not able to confirm the transaction", with two approved claims reading *Ready to submit for confirmation* and nothing to press. Nothing was broken: spec 041 had moved the act one tab across, and the tab left behind was still **called** "Payments confirmation" and still **instructed** "transfer the covered amount, then confirm it here (the employee is emailed)" — an instruction that had been false since the money moment became the bank confirmation. A person reading a screen that names their job, tells them to do it there, and offers no control concludes the product is broken; that is a correct reading of what the screen says. The actual blocker was a data condition the screen also declined to state — the unit had nobody appointed — so it presented as a missing button on **both** tabs. Three rules. (a) Moving a step means editing the old screen in the same commit: its title, its instruction, and a pointer to where the work now happens. (b) A row that cannot be acted on says **why on the row** — "nobody is appointed to confirm this unit" — never a silently absent button, because absence is indistinguishable from a bug. (c) When a redesign dissolves a structure that was silently enforcing a rule, the rule must be re-implemented **explicitly and named in the code**: the per-unit bands made a two-unit submission impossible by there being no list containing two, and one searchable table gave that away, so the guarantee came back as a selection lock (`AwaitingTable`) — the server's own `sameBusinessUnit` refusal being what made the swap safe to attempt at all. And the corollary that found all of this: with no access to the live database, the way to tell a data condition from a bug is to **rebuild the schema on a throwaway Postgres and drive the real app in a real browser** — the queue, the send, the confirmation and the resulting REIMBURSED row — rather than reasoning from the code, which read correctly the whole time.
- **A `redirect()` to report a one-row change throws the whole page away (2026-09-08)** — every appoint/remove action on Who moves money ended in `redirect(BACK?ok=…)`, so appointing one person discarded the screen, rebuilt six units of it and dropped the operator at the top, to say one sentence about one row. The CEO: *"the appointment needs to be by cell not for the whole page like that."* An action whose effect is local returns a **result the row renders**, and `router.refresh()` re-renders the server page in place — scroll kept, nothing else moving; the page-level green banner goes with it, because a confirmation belongs beside the control that earned it and not above six units it says nothing about. Two things this drags in: the result type lives in a **plain module**, since every export from a `"use server"` file must be an async function; and the proof that it worked is **scroll position measured either side of the click**, not the absence of a visible flash.
- **A QUEUE OF THINGS TO DO NEEDS A "DONE" STATE ON THE THING, not an absence somewhere else (2026-09-08)** — the day the payments table shipped, the CEO: *"all the petty cash was released already."* Seventeen float top-ups going back to November 2024 — the imported marketing history, whose own rows are annotated *"Reimbursed to the custodian at the end of the period"* — were sitting in Finance's queue as **~EGP 285,000 still to be transferred**, and the header tick selected all seventeen at once. `PettyCashFunding` had **no paid/unpaid state whatsoever**: a payback has a status, a benefit claim has a status, a funding row had nothing, so `availablePayables()` could only ask "is this a top-up with no live batch item?" — true of every top-up ever recorded. The absence of a batch item was standing in for "unpaid", and it never meant that. Nothing would have corrupted (completing a transaction writes back to paybacks and claims, not to petty cash) and that is exactly why it would have got through: the only casualty is the confirmer being emailed a total and asked to release money that left months ago. Rules: (a) **before a record type joins a work queue, ask what field says it is finished** — if the answer is a join to somewhere else being empty, that is a proxy, and a proxy is wrong the first time the record arrives by any other route (here, an import); (b) an event that is *"has this happened yet"* is a **nullable timestamp on the record**, so it answers *when* as well as *whether*, and the queue's filter reads as the sentence it means (`transferredAt: null`); (c) a **migration that backfills a new state column must key off the COLUMN APPEARING, not off the rows' current values** — "stamp everything still null" is right exactly once and catastrophic on a replay, because null becomes the normal state of the very thing the feature exists to track, so replaying would silently empty the queue of real money owed. That last one was written the obvious way first and found by **re-running the file against a database that had since queued a payment** — a migration is not idempotent because it says so at the top; it is idempotent when a second run has been watched.
- **A screen lives where its USER already goes, not where their ROLE suggests (2026-09-08)** — spec 041 gave the confirmer his own route and sidebar entry, reasoning from his role: he is not a Finance user, so he should not have to enter Finance's workspace. The confirmer is the CEO, and he goes to Payments to watch money move: *"the confirmations of transactions is appearing in the external panel which is not correct, it can be part of the payments panel as a subtab for me to go and confirm through."* The screen is now a tab on Payments and nothing on it changed. Three things a move like this drags in: the **door** becomes a union (`canOpenPayments` — Finance OR the appointment), asked by the layout and the page alike, so an appointee who is not Finance still gets in and sees only their tab with none of Finance's queries run; the **old addresses redirect** rather than 404, because an email already sent cannot be edited and both confirmation emails linked there; and an address must be able to **name the tab** (`?tab=`, with an unknown id falling back to the first), or the email's "see the transactions" lands on the wrong queue. Proven by driving three people through it in a real browser: the CEO confirming, a non-Finance appointee returning one to Finance, and Finance-without-appointment being refused the tab and the page.
- **A REFUSAL MUST SAY WHETHER THE PERSON IS WRONG OR THE SYSTEM IS BROKEN — never blame the user for our outage (2026-09-14)** — reported as *"all team is getting wrong password messages"*, which read as a password-data problem and is not one. The sign-in page mapped **every** `AuthError` to *"Incorrect email or password"*, and Auth.js raises an `AuthError` for two completely different things: `CredentialsSignin` (authorize returned null — genuinely the wrong password) and `CallbackRouteError` (authorize **threw** — in practice a Prisma `P1001`, the database unreachable). Measured in a real browser: with the **correct** password and Postgres stopped, the old code printed the identical wrong-password sentence, so a company-wide database outage and a typo were indistinguishable on screen **and** in the logs. Two rules. (a) An error message must be derived from **which** failure occurred, not from which screen it happened on — a catch-all `instanceof` over an error family whose members mean opposite things is the same fault as a count that isn't its own choice's count. (b) The wrong version of this message is actively harmful, not merely unhelpful: it sends the whole company to HR for a password reset that cannot possibly work, and it makes the one diagnosis that matters ("everyone at once ⇒ one shared cause") the one the screen argues against. Anything that is not a credential mismatch now says the system can't be reached and that resetting won't help, and the real cause (`err.type` + `err.cause`) is logged where an operator can find it — the screen itself stays vague, since it must never hint at whether an account exists. Corollary for `authorize()`: do **not** wrap its database reads in a try/catch that returns null — swallowing the throw is precisely what turns an outage back into "wrong password".
- **A DRAG ON A LIST THAT REORDERS ITSELF MUST LISTEN ON THE WINDOW, AND MEASURE ONCE (2026-09-15)** — reordering the Learning courses is a drag handle per row, and the three things that broke it were all invisible to `tsc`, to `next build`, and to reading the code; each was found only by driving a real browser against a real Postgres. (a) `setPointerCapture` on the handle is the obvious way to follow a pointer and it **loses capture on the first swap**: React re-sequences a list by MOVING the `<li>` nodes, and an element that leaves the document drops its capture — so the handle stopped receiving moves, the last position the component ever saw was the one just before the first reorder, and the drag froze half way with the row still lifted and following nothing. Every time, looking like a placement bug rather than a lost-listener one. The listeners belong on the **window**, which cannot be reordered out from under them. (b) Placement computed from **live** `getBoundingClientRect()` lags a pointer moving faster than the browser repaints, because each swap re-lays-out the list and the next event measures the PREVIOUS arrangement: the identical drag landed correctly at 60ms between moves and three rows short at 25ms — and a real mouse reports roughly every 8ms. Freeze every measurement at the start of the gesture and make the rest arithmetic. (c) A pointer **cannot leave the viewport**, so a row below the fold has no midpoint the pointer can get past and a long drag silently stalls at the last visible slot; edge auto-scroll fixes it, and must be **clamped to the page height captured before the row was lifted**, because a translated row lengthens the document and an unclamped scroll finds fresh "bottom" on every frame and runs away into blank space under a still finger. And the one that cost the least and should have: a context created but never **provided** renders every consumer as nothing, and a list of rows with no handles looks like a finished page.
- **A DERIVATION NOBODY CALLS IS NOT A RULE, IT IS A FILE (2026-09-16)** — the whole point of Learning tracks is that a track states which courses matter first, and `sequenceForLearner` was written to say so, in the one module that already owns ordering, with the reasoning in its comment. It was never called. The employee's page therefore showed the company-wide order the whole time, while every stored figure was right, `tsc` was clean, the build was clean and 71 checks against a real Postgres passed — because each of them asked whether the DATA was correct, and the data was. Nothing asks whether a function is reached. Two rules out of it: when a derivation is extracted, the commit that extracts it is the commit that calls it, and the check that proves it is an assertion about the OUTPUT SOMEBODY SEES (here: the courses come back in the track's order, deliberately seeded so the track's order differs from the company's — seed them the same and the test passes either way); and drive the actual screen, because the only evidence that existed was a list in the wrong order in a browser. Three smaller ones from the same build, each cheap to re-learn the hard way. (a) **A new scheduled job is gated by the MODULE switch too, not just its own** — the reminder cron was behind the platform toggle and Learning's own, and would still have emailed people about courses while the module was off and its page redirected to the dashboard: chasing somebody for work the product refuses to let them do. Ask which doors close this feature, not which switches it owns. (b) **"Who was assigned" and "who is on it" are different questions and need different panels** — the assignee list deliberately leaves a group as a group (assigning a group is a statement about the group), which is exactly why it cannot also be where a person's progress is read; and the headline count must come from the expanded, de-duplicated set, or somebody named directly who is ALSO in an assigned group is counted twice. (c) **A link offered to everyone that only some may open is a silent bounce** — the roster's door to a person's plan is shown only where the viewer would actually get in, because whoever runs Learning is not automatically anybody's manager, and a link that redirects reads exactly like a broken product.
...
### 3c. Known data checkpoints — look here FIRST when a stored value looks wrong

A fixed bug stops new bad data; it does not clean up what the bug already wrote. When a figure or a
date on screen looks wrong and the code reads correctly, check this list before hunting: the answer
may be that the row was written by the old behaviour. Each entry says what to suspect, how to tell,
and how to fix one. **Add an entry whenever a fix leaves existing rows behind, and delete it once
the data has actually been checked** — a stale entry sends the next session down a dead end.

- **Incentive: a start or closure date read the American way round, on anything uploaded before
  2026-08-25.** Until that day the module read a sheet date through `new Date(...)`, which is
  **month-first** — so an operator's `01/03/2021` (1 March) was stored as **3 January**, silently:
  no warning, and nothing on screen looked odd. Fixed in `src/lib/incentive/dates.ts` (day-first,
  and the module now reads and writes `14-Jul 2026`), but **rows written before the fix were not
  migrated** — the CEO's call, 2026-08-25: *"all is done now for previous cycles and we can figure
  out the anomaly on the go."*
  - **Suspect it when:** a cycle's assignment appears to start or close in the wrong month, an
    assignment looks like it belongs in another quarter, or a person's start date is a few months
    out — **and only where both numbers are ≤ 12** (a `22/07/2021` was always read correctly, so
    only days 1–12 can have flipped).
  - **Tell for certain by:** opening the original sheet next to the cycle's Review & validation
    table. A flipped date is the same two numbers swapped — `03/01/2021` where the sheet says
    1 March. Dates now display as `14-Jul 2026`, so a swap is visible at a glance.
  - **Fix one by:** editing the cell in place and pressing **Recalculate** — no migration needed.
  - **Money was not affected.** Payability comes from the assignment's status, not its dates, so no
    payout figure was ever mis-stated by this; it is a reporting/attribution problem only.
...
- **A broadcast is not a transactional email and must not inherit its stance** — the claim and
  ...
  confirmation.
- **Email is limited to SIX workflows (specs 020 + 035 + 037 + 039 + 040 + 043)** — the original "no emails, ever (v1)" rule was reversed for the benefit-claim workflow (approved 2026-08-10), widened to the holiday/vacation workflow (approved 2026-08-19), widened twice more on 2026-08-24: to **Team Communications** (announcements to a chosen audience, plus personal congratulations for birthdays and joining anniversaries) and to the **payback workflow** (requested by the CEO: submitted→Finance, declined/paid→the requester), and on 2026-09-08 to the **time-off request cycle** (CEO: *"someone requested a vacation and I didn't get the email"* — new request→the approver(s) whose queue it landed in, decision→the requester; spec 035 FR-013), and on 2026-09-16 to **Learning track deadlines** (spec 043: a course whose deadline passes unfinished→the employee, and their manager). All send via **Resend**, **env-gated** (`RESEND_API_KEY`/`EMAIL_FROM`), configurable + master-toggleable at **Admin → Notifications**. The transactional workflows are **fire-and-forget** (never block a state change); the broadcast one reports per recipient (see the rule above). **An email's recipients are derived through the same rule as the screen that shows the thing** — the leave-request email asks `leaveApproversFor`, the inverse of `pendingApprovalWhere`, so the person whose queue just grew is exactly the person told. **A scheduled process MAY now email an employee — about an obligation that is THEIR OWN, and never employees at large (constitution 2.0.0, 2026-09-16).** This REVERSES what stood here from 2026-08-19: *"no scheduled process may email an EMPLOYEE — cron prepares drafts and nudges operators, a human sends"*, a clause the constitution marked NON-NEGOTIABLE and restated verbatim through four widenings. The CEO's reason: **a deadline nobody is reminded of is not a deadline.** The old rule's value was having no exceptions, so the new one carries its limits — all of them required, not aspirational: **bounded** (at most five messages about one overdue thing — one on the day, then weekly for four weeks, then silence, after which it stays visible on screen but stops arriving); the **bound fixed in the feature, never a settings box** (a bound nobody decided gets widened by the first person annoyed that work is unfinished); **never twice** (sends are recorded, so a job that runs twice or retries cannot email twice); **fire-and-forget**, with a failed send never recorded as a successful one; **stops the moment the obligation is met**; the sending module carries **its own off switch** beside the platform toggle, which may only ever NARROW (it can silence mail the platform allows, never enable mail the platform disabled); and **silencing the chasing must not hide the fact** — the overdue state still shows to the person, their manager and whoever runs the module either way. A company-wide ANNOUNCEMENT is still drafted by the platform and sent only by a human who read it. Still **no** invitations, marketing, external recipients, or scheduling a send for later. **Petty cash itself sends nothing** — the custodian and Finance are both looking at a live screen, and a notification for a ledger nobody is waiting on is email surface bought for no one's benefit.
...
*Last Updated: 2026-09-16 (Learning tracks shipped (spec 043) — a named path of courses that GRANTS what it contains, set centrally, with a manager able to add for their own reports but never to remove a company requirement, deadlines of either kind per step, and a bounded daily chase. Added from building it: a derivation nobody calls is not a rule but a file — the sequencer that puts a track's courses in the track's order was written, commented and never called, so the employee's page showed the company order while every stored figure was right and the type check, the build and 71 database checks all passed; the proof has to be an assertion about what somebody SEES, with the fixture deliberately seeded so the two orders differ. Plus: a new scheduled job is gated by the module switch too, or it chases people for work the product refuses to let them do; "who was assigned" and "who is on it" are different questions needing different panels, and the headline count comes from the expanded, de-duplicated set or somebody reached twice is counted twice; and a link only some viewers may open is shown only to them, since a silent bounce reads as a broken product. Previously the same day: Constitution amended to **2.0.0** — the first MAJOR. Email widens to a sixth workflow (Learning track deadlines, spec 043) and with it the NON-NEGOTIABLE "no scheduled process may email an EMPLOYEE" is REVERSED, narrowly and conditionally: a schedule may remind ONE person about ONE obligation THEY hold, bounded at five messages, never twice, stopping when the obligation is met, behind a module switch that can only narrow — and never employees at large, where a human still reads and sends. The CEO's reason, recorded because the reversal is only defensible with it: a deadline nobody is reminded of is not a deadline. The superseded clause is quoted in place rather than deleted, since whoever reads it next is owed the rule it replaced. Corrected in the same pass rather than silently: the constitution's footer said 1.4.0 while its body already carried the 2026-09-08 time-off widening, so that missing bump is recorded honestly as 1.5.0. Previously: 2026-09-15 (Added, from giving the Learning admin a handle to arrange the courses with: a drag on a list that reorders itself must listen on the WINDOW and measure ONCE — pointer capture is lost the moment React moves the `<li>` nodes, so the drag froze half way every time; placement read from live rectangles lags a pointer moving faster than the browser repaints, landing three rows short at 25ms between moves when a real mouse reports every 8ms; and a pointer cannot leave the viewport, so a row below the fold is unreachable without edge auto-scroll, which must be clamped to the page height taken before the row was lifted or it runs away into blank space. All three were invisible to the type check and the build and were found only by driving a real browser. Previously: 2026-09-14 (Added, from the whole team reporting wrong-password messages at once: a refusal must say whether the person is wrong or the system is broken — the sign-in page mapped every auth error to "incorrect password", including the one Auth.js raises when the database is unreachable, so a company-wide outage and a typo printed the same sentence and sent everybody to HR for a reset that could not help; proven by driving a real browser with the CORRECT password and Postgres stopped. A credential mismatch keeps its wording; anything else now says the system can't be reached, and the real cause is logged where an operator can find it. Previously: 2026-09-08 (Added, from the CEO calling the separate Confirmations entry "the external panel": a screen lives where its user already goes, not where their role suggests — the confirmer's queue is now a tab on Finance → Payments, the door is Finance OR the appointment so a non-Finance appointee still gets in and sees only their tab, the old addresses redirect because sent emails link to them, and an address can name the tab to open on. Earlier the same day, from seventeen already-reimbursed petty cash top-ups sitting in Finance's queue as ~EGP 285,000 still to be released: a queue of things to do needs a DONE state on the thing itself — a funding row had no paid/unpaid state at all, so "no batch item yet" was standing in for "unpaid" and was wrong for every row that arrived by import; the state is now a nullable transferred-on timestamp, so the queue's filter reads as the sentence it means. And a backfill for a new state column must key off the COLUMN APPEARING, never off the rows' current values — "stamp everything still null" is right exactly once and would, on a replay, silently empty the queue of real money owed; found by re-running the file against a database that had since queued a payment, because a migration is idempotent when a second run has been watched, not when it says so at the top. Earlier the same day, from Finance reporting they could not confirm a transaction: when a step MOVES, the screen it left behind has to say where it went — the tab was still called "Payments confirmation" and still said "confirm it here" a fortnight after the act moved one tab across, so a person reading it correctly concluded the product was broken, while the real blocker (a unit with nobody appointed) was a data condition neither screen would state; a row that cannot be acted on now says why ON the row, since an absent button is indistinguishable from a bug. Plus, from turning that queue into one searchable table: a structural guarantee dissolved by a redesign must be rebuilt explicitly and named — one band per business unit had made a two-unit submission impossible by there being no list containing two, and the table gave that away, so it came back as a selection lock. And the method that separated data from bug with no access to the live database: rebuild the schema on a throwaway Postgres and drive the real app in a real browser, rather than reasoning from code that read correctly the whole time. Plus: a `redirect()` to report a one-row change throws the whole page away — appointing one person rebuilt six units of screen and returned the operator to the top, so a local action returns a result the row renders and `router.refresh()` updates in place, proven by scroll position measured either side of the click. Previously, the same day: Email widened to a fifth workflow at the CEO's request after a vacation request sat unnoticed: the time-off cycle now emails the approver(s) on a new request and the requester on the decision — spec 035 FR-013; recipients are derived through the same rule the approval queue uses, so the email and the badge cannot disagree about who is asked. Previously: 2026-09-01 (Added, from the whole team's benefit claims dying as a raw error page: an upload's real ceiling is the smallest limit anywhere in the path — Next.js caps server-action bodies at 1MB by default, under every cap the forms advertise, so any receipt over 1MB died before the action's own checks could run; the body limit is now configured above the largest advertised upload, the claim forms shrink a large photo in the browser and refuse an oversized proof inline before dispatching, and the dispatch itself is caught so a transport failure answers in words instead of crashing the page. Previously: 2026-08-26 (Added, from releasing incentive payments into Finance: a guard must start from the thing it protects rather than the thing being checked — the released-figures check walked the report's lines, so deleting a person produced no line, nothing to check, and money already gone could be edited away; and a file the type checker never sees is not code but a wish — `**/*.ts` does not match `.mts`, so all 21 verification scripts had never been checked, and switching it on surfaced two pieces of breakage from that same day in a script whose whole job is to catch breakage. Previously: a hand-written list is a checklist somebody has to remember — Payback had a nav entry, a page and a Finance queue but no row in the Modules switch, and a list of eight looks complete, so nothing said so; a module is only under the switch once it is in the array AND its page and write actions call the guard, and a switch covering only part of a module says which part in the list itself. Previously: 2026-08-25 (Added: a "Known data checkpoints" section — a fixed bug stops new bad data but never cleans up what it already wrote, so any fix that leaves existing rows behind gets an entry saying what to suspect, how to tell and how to fix one, deleted once the data has actually been checked; its first entry is the incentive dates written month-first before today. Also, from putting the incentive module onto `14-Jul 2026`: where a person checks their own typing, spell the month — a deliberate, scoped exception to the platform's dd/mm/yyyy, with one module formatting AND parsing so the four places a date appears cannot disagree; a native date input cannot carry a house date format — it draws itself in the browser's UI language, mm/dd/yyyy under en-GB, ar-EG and en-US alike — so an entered date is typed text stating its format; a stored date-only string is printed by reordering the string, never by parsing it, or a viewer behind UTC sees the previous day; and `new Date("01/03/2021")` is American, so a day-first importer is the only safe one — this one had been storing 1 March as 3 January in silence. Also, from making the incentive review tables editable in place: every export from a `"use server"` file is an endpoint, so a raw write belongs in a plain module and the action keeps the access check; and a `useActionState` dispatch fired from a plain button needs `startTransition` by hand, or React never reports pending and the button neither says "saving" nor blocks a second click — invisible to `tsc` and to the build, caught only in a real browser. Also today, from the Learning course page 500-ing on every action: a `"use server"` file may export nothing but async functions, and one stray export breaks EVERY action on the page — Next validates the whole action entry on the first call, so a single exported array turned the visibility switch, adding people and Publish into three bare 500s with no message; nothing catches this, since it is not a type error and `next build` compiles it happily. Plus: a hover `transform` makes a card its own stacking context, so a menu inside it cannot escape — raise the card, not the popover. Both were found only by driving a real browser against a real Postgres, neither is visible in the code or a type check. Also today, from adding rename and delete to courses: a course's FILES must be deleted with it, since every row cascades but nothing would then reference the blobs; a refusal belongs BEFORE the confirmation, not after it; and an update must leave a field the form did not carry alone rather than reading absent as cleared. Previously, from splitting bank confirmations by business unit: a shared control must be scoped to the thing it actually guards — the scope is derived not typed, the screen makes mixing structurally impossible and the server re-checks anyway, and there is no appointment meaning "all of them"; plus, drop the old uniqueness BEFORE widening it, or ON CONFLICT DO NOTHING eats the migration in silence — two appointments across five units produced 2 rows instead of 10 and reported success, so check a migration by the count it should produce, never by whether it errored. Also today, from combining Announcements into Communications: reuse a field when it is the same FACT, not the same shape — the email header named the platform because one column was serving two ideas, and the only fix was to rename the application; a screen must never claim work is saved that is not, so track dirty rather than saved and make two controls that can disagree into one; and a fix that removes a scrollbar must say where the content went — removing the vertical one clipped the right edge, and the frame's own border then clipped it again by 2px, both caught only by driving a real browser. Earlier the same day, from importing the marketing petty cash workbook: a parse is only as good as the total it is checked against — two silent bugs, a doubled month and a lost column of receipts, both caught by comparing each tab against the sheet's own figure rather than by reading the output; read a spreadsheet's cells, not its labels; report a source's own errors rather than absorb them; and a generated migration is reviewed through its generator, so that is what gets committed. Plus: a receipt that lives somewhere else is still one door — evidence gained an external location and the serving route redirects after making the same 404-not-403 decision. Previously, from Reviews & 1:1s (spec 042, renumbered from 040 on merge — Finance had taken 040/068 first): a privacy promise must survive "view as", so the module resolves the REAL session user and refuses to run while impersonating; whose records these are is stored, not derived — the deliberate opposite of the Time-Off rule, because a review belongs to the two people who had it; and sealing means the data is not sent, not that it is hidden — the counterpart's rows are never fetched, so there is no preview or completion state in the payload to uncover. Previously: nobody is told money arrived until it has — the two "your money is here" emails now fire on the CEO's bank confirmation, not on Finance recording a transfer; say what the person does in their words, because two wrong verbs produced two wrong designs; an appointment's role-holders are implicit except where that would make a promise false; a figure a decision is made against is frozen, not derived; an enum member's position must be checked against the real database; an already-applied migration never runs again, so ship the delta as a new file that converges any stale database; one signed reconciliation figure; money as Decimal in Postgres and piastres in TypeScript; a lock that protects an invariant you can name; an acknowledgement that says what was accepted; and talk like a person, not a system. Merged the same day with Team Communications — a colour an operator chooses must carry its own legibility rule, and a broadcast must not inherit a transactional email's stance. Previously: how to actually run the `verify-*.mts` scripts — `apply-sql.mjs` after `db push`, or the reference data several of them read is simply absent; and the rule that a script sharing a database must never assert a number about the whole of it, because four order-dependent failures in one sweep all came from two scripts reaching for the same department name or the same fixture ids. Previously: 2026-08-23 (Added: a gate is not a scale — the sub-6-month pool ceiling now scales to the cycle like everyone else's, and a figure shown beside a decision counts only what that decision moves, through the derivation the write uses. Previously: 2026-08-22 (Added: a count shown beside a choice must be that choice's count, computed through the same derivation the real check uses; one way to say "everyone". Plus: per-module authority is an appointment, never a new Role member — one derivation, role-holders are never rows, the appointment cannot appoint. Plus: a visibility rule needs one source and must be re-decided at the serving route, 404 not 403. Previously: 2026-08-20 (Added: `npm test` is a tool, not a routine — no regime, no deploy gate, no standing obligation; protection is structural. Plus: the pool ceiling is enforced on every write path in every order — one derivation, signed remaining, refuse-don't-clamp, per-employee row lock; the freeze-vs-parked-header table rule; unchanged legacy identity values no longer block an unrelated employee-form save, and rejected saves are now scrolled to / announced / listed in full. Previously: official-holiday lifecycle + announcements (spec 037) and the first scheduled job; email widened to that workflow; HR may reopen a rejected claim with a reason. migrations now run through Claude via the deploy, not by hand; added the no-unneeded-complications rule.)))))))*
