...
**v1 modules:** Foundation (auth + roles + employee registry + **My Documents** personal uploads) · Onboarding · Benefits · Team Directory · **Handbook & Resources** (shared policies/handbook + downloadable company files) · Time-Off / Leave Management · Dashboard · Learning Track · **Reviews & 1:1s** (spec 039).
...
- **A privacy promise must survive "view as" (spec 039, 2026-08-24)** — `requireUser()` deliberately
  ...
  **refuses to run at all** while impersonating. Rule: before promising that something is private,
  ...
  inherits the previous one's conversations. Both departures are recorded in spec 039's
  Complexity Tracking so a later session does not "fix" them back.
- **Sealing means the data is not sent, not that it is hidden (spec 039)** — a review sheet's two
