...
- **A "done" state is derived from the record it points at, never stored beside it (spec 040, 2026-08-25)** —
  a journal note flagged to raise leaves the queue because a 1:1 note or a sheet item **references
  it** (`carriedJournalRefs`), not because a `raisedAt` column was set. A stored flag is a second
  source of truth that drifts the moment the thing it describes is deleted: the note would still read
  "raised" with nothing to show for it. Derived, deleting the raised note honestly returns the entry
  to the queue. Same family as the pool ceiling and `audienceReachByRule` — compute it through the
  record that actually decides it.
- **A prompt belongs where the decision is, not in a badge (spec 040)** — flagged notes wait at the
  top of a 1:1 and on the review sheet, and nowhere else: no count on the home page, no reminder, no
  email. A module built so that nobody oversees it must not grow a nag that oversees it anyway.
