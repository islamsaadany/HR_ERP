...
### 2026-08-17 — Idempotent answers + sitting store + campaign delete (third live-testing round)
- **Answering is idempotent.** The server accepts an answer for ANY field an open campaign requested — pending or already settled — and the latest answer wins (re-confirm re-confirms, re-edit updates). This kills the two dead-ends found live: "Nothing was filled in" when editing a field a refresh had already settled, and "There is nothing pending to answer" when confirming one.
- **The sitting store.** The popup keeps its field list and answered chips in a per-tab module store, so no framework remount or refresh can reset an answered row to actionable or make a row vanish mid-sitting. Resets on a full page load (a new sitting shows only what's still pending).
- **Campaigns can be deleted** from the Data Requests list and the tracker (explicit confirm) — the tracker history is erased and popups stop; answers already written to profiles stay.
- Verified against a **production build** (not dev): 20 scenario checks — confirm/save/Enter isolation per field, chips surviving revalidation refreshes, edit-cancel, invalid-confirm → row error → fix, a forced externally-settled field answered without dead-end, dedup across two campaigns, Finish/Later, tracker outcomes, delete. Zero unexpected console errors.

### 2026-08-17 — Live delivery (fourth live-testing round: the dead badge)
- **Root cause**: layouts do not re-render on client-side navigation, so a campaign launched while an employee's session was already open never reached the popup — the server-rendered badge showed a count, but the frozen (empty) popup made the badge click do nothing and no popup appear.
- **Fix**: the popup layer now OWNS its data: it polls `/api/profile/data-requests` on mount, on tab focus, and every 30s, merges new asks into the sitting (auto-opening), and broadcasts the live pending count to the sidebar badge — which now also clears live when everything is answered. The layout prop only paints the first frame.
- **Mistaken confirmations**: an answered row keeps an Edit button — re-open, change, re-save (the server takes the latest answer; the tracker shows Corrected). Cancelling a re-edit restores the chip exactly. Outside the popup, My Profile and HR's admin form remain the standing correction paths.
- Verified in production mode: campaign launched mid-session reached an open session with no reload (popup auto-appeared on tab focus), the badge appeared live with the right count, its click opened the popup, the mistaken-confirm → re-edit → Corrected flow round-tripped to the tracker, and the badge cleared live after finishing. 14/14 checks.
