...
- **FR-018**: The system MUST restrict HR-private fields (employment type, tenure band, start/end date, status, date of birth, marital status, dependants, detailed reporting data) to HR Admin / Super User, enforced server-side. **Monthly salary is confidential and further restricted to Super User only** — HR Admin MUST NOT be able to see or edit it (the grid column, form field, inline edit, and server writes are all Super-User-gated, and salary is never sent to a non-Super-User client).
