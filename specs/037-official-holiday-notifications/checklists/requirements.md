...
- Edge-case verification round (2026-08-19): all eight open edge cases were put to the user as explicit decision questions and their answers encoded (recount live; auto-zero + notify on bridge collision → FR-017; HR-sent corrections → FR-018; past edits allowed with warning; weekend clashes allowed with honest text; multi-day holidays as ONE grouped entry — user's choice over per-day entries; overlapping actual ranges blocked; re-fetch diff with apply-as-move → FR-016).
