...
**Input**: Feature specification from `/specs/040-transaction-approval/spec.md`

## Summary

Finance groups what they entered in the bank into a **batch**, sends it, and the appointed confirmer
— the CEO — is emailed with a total and a link. He confirms the transfers in the bank as he always
has, then ticks the batch off here; only then are the people being paid told. A **salary run** is the
same batch carrying a month, a total and a headcount, with no per-person figure anywhere.

Nothing here gates a payment. The bank does that, on two signatures. This feature is the
notification and the record — the framing the CEO corrected on 2026-08-24, and the reason every name
in this plan is *send*, *confirm* and *tick off* rather than *approve*.

Spec 039 did the groundwork: the money helpers, the access derivation, the refusal pattern, the
private-evidence route, and the two reserved columns these batches attach to. This plan adds four
models, one enum member, five surfaces and one scheduled reminder.
...
send/confirm rules get unit coverage because they decide who can release money
...
**Performance Goals**: A handful of batches a month, tens of items each. No pagination needed

**Constraints**: No payee name or amount may appear in an email. No individual salary may be stored,
shown or exported. Email never blocks a state change. A confirmed batch is immutable

**Scale/Scope**: ~10 batches/month, ~12 salary runs/year, 1–2 appointed confirmers. 4 new models,
...
| **III. Money & Rules Server-Authoritative** | ✅ Pass | Who may send, who may confirm, and the frozen total are all decided server-side. The batch total is computed once at send time and stored; nothing recomputes it later, so the figure emailed and the figure confirmed cannot diverge. |
...
| **V. Engineered Enough, Explicit Over Clever** | ✅ Pass | One derivation for confirmation rights, one for batch state; refusals as sentences; edge cases mapped to concrete messages in `contracts/`. |
...
| **Email limited to three workflows** | ✅ Pass | This adds messages to the third (finance), which spec 039 already established. No fourth workflow. |
...
```text
specs/040-transaction-approval/
...
└── sql/068_payment_batches.sql         # idempotent, same commit

src/
├── lib/finance/
│   ├── confirmers.ts                   # THE derivation: may this person confirm?
│   ├── batches.ts                      # pure: what a batch totals, what it may become
│   └── access.ts                       # + canSendBatch / canSeeSalaryRuns
├── lib/email/templates.ts              # + batchSentToConfirmer, batchWaitingReminder
├── app/
│   ├── (app)/
│   │   ├── confirmations/              # the confirmer's screen — one list, one tap
│   │   │   ├── page.tsx
│   │   │   ├── [batchId]/page.tsx
│   │   │   └── actions.ts              #   confirm, send back
│   │   ├── finance/
│   │   │   ├── page.tsx                #   + "Sent to the bank" sub-tab
│   │   │   ├── batch-actions.ts        #   create, send, withdraw
...
└── components/confirmations/           # BatchList, BatchDetail, SalaryRunForm
```
...
workspace to do a ten-second job; the email links straight to it. Finance sees the same batches from
their side, under the Payments page they already use.

## Key design decisions

1. **The total is stored, not derived.** This is the one place in the Finance module where a figure
   is deliberately *not* recomputed on read. A batch's total is what the confirmer was emailed and
   what he confirms; recomputing it later would let those two diverge, which is precisely the failure
   this feature exists to prevent. Items are locked while a batch stands, so the stored figure cannot
   go stale.
...
3. **One enum member, positioned deliberately.** `PaybackStatus` gains `PAYMENT_SUBMITTED` between
   `APPROVED` and `PAID`, exactly as spec 039 reserved. In Postgres that is an `ALTER TYPE … ADD
   VALUE … BEFORE 'PAID'`, which must run outside a transaction block — noted in the migration.

4. **Two payable kinds, one join table.** A batch item points at either a payback request or a float
   movement, using the `paymentRunId` columns 039 reserved. The item row carries the amount **as
   sent**, so the batch's history survives even if a source record is later corrected.
...
| **G2 — Mockups approved** | The confirmer's screen, the batch detail, Finance's send screen and the salary run published and **explicitly signed off**. No component before this passes. |
...
| A batch's total is stored rather than derived | The emailed figure and the confirmed figure must be the same number by construction | Recomputing on read, the house default — rejected because it reintroduces exactly the drift this feature prevents |
