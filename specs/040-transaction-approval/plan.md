...
Spec 039 did the groundwork: the money helpers, the access derivation, the refusal pattern and the
...
3. **One enum member, positioned deliberately.** `PaybackStatus` gains `PAYMENT_SUBMITTED` where
   spec 039 reserved room. In Postgres that is an `ALTER TYPE … ADD VALUE`, which must run outside a
...
4. **Two payable kinds, one join table.** An item points at either a payback request or a float
   ...
   later correction to the source record. (The `paymentRunId` columns 039 reserved turned out to be
