...
Apply `prisma/sql/071_performance_reviews.sql` to it **twice** — it must be idempotent, because the
...
- Migration `071` applied by the deploy; the build log's `[apply-sql]` lines checked and the outcome
