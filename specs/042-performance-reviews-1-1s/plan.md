...
**Input**: Feature specification from `/specs/042-performance-reviews-1-1s/spec.md`
...
`prisma/sql/071_performance_reviews.sql`, idempotent, committed with the schema change.
...
| **Migrations are Claude's job** | **PASS** | `071_performance_reviews.sql`, idempotent, same commit; applied by the deploy-time runner; `[apply-sql]` lines checked and the result reported in one line. |
...
```text
specs/042-performance-reviews-1-1s/
├── plan.md              # This file
├── spec.md
...
└── sql/071_performance_reviews.sql
```
...
2. Schema + migration `071` + 34-theme seed; verified against a throwaway Postgres.
