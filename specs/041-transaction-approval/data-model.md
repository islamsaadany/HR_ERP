...
Target: `prisma/schema.prisma` plus the idempotent `prisma/sql/069_payment_batches.sql` in the same
commit. Money is `Decimal(10,2)` read through `src/lib/finance/money.ts`, exactly as spec 040.
...
**One member added to an existing enum** (the position spec 040 reserved):
...
## Migration notes (`prisma/sql/069_payment_batches.sql`)
...
- It also **drops** the two `paymentRunId` columns spec 040 reserved. Membership landed in
