...
**In scope**: the confirmer appointment, submissions over the payables spec 040 creates, the monthly
...
an unrecorded confirmation matters most. It needs nothing from spec 040 — no payback request and no
...
- **FR-006**: Finance MUST be able to tick payables — approved payback requests, petty cash funding
  movements (spec 040) and **approved benefit claims awaiting reimbursement** (spec 020) — and
...
- **FR-036** *(2026-09-08)*: The confirmer's queue MUST be a tab on Finance → Payments — named
  ...
  *Payments confirmation* tab) — not a separate surface — the CEO: *"it can be part of the payments panel as a subtab for me to go and
  ...
  still get in, seeing that tab alone.
...
- **This changes one step of spec 040**: a payback request becomes **Paid** when the submission
  carrying it is marked complete, not when Finance records the transfer. That single added state —
  *at the bank, awaiting confirmation* — is the whole of 040's rework.
...
- **Depends on spec 040** for the payables a submission groups. User Story 3 (salary runs) and User
...
- **Constitution amendments required**, recorded alongside the code:
  1. **Scheduled work** is currently described as one daily job that may nudge HR. FR-022 adds a
     ...
     at large.
  2. The email clause already permits three workflows after spec 040; this adds the
