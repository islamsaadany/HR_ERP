...
- [x] T003 Add the five enums to `prisma/schema.prisma`: `PettyCashAccountStatus`, `PettyCashPeriodStatus`, `PettyCashPaymentMethod`, `PettyCashFundingType`, `PaybackStatus` — with the comment recording that spec 040 inserts `PAYMENT_SUBMITTED` and the order must not change (data-model.md).
...
- [x] T015 Record the approved version in `specs/039-petty-cash-payback/plan.md` (gate G2 → passed, with the mockup filenames), and fold any requested changes back into the mockups before proceeding.
...
- [x] T049 [P] Update `IMPLEMENTATION_PROGRESS.md` with spec 039 as built, and `IMPLEMENTATION_PLAN.md` if any decision changed during implementation.
- [x] T050 Set `specs/039-petty-cash-payback/spec.md` **Status** to Implemented and tick `checklists/requirements.md` where implementation confirmed it.
