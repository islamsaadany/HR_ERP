...
**Input**: Feature specification from `/specs/039-petty-cash-payback/spec.md`
...
| **Migrations are Claude's job** | ✅ Pass | `prisma/sql/067_petty_cash_payback.sql`, idempotent, committed with the schema change; the deploy log's `[apply-sql]` lines are checked and reported. |
...
└── sql/067_petty_cash_payback.sql     # idempotent, same commit
...
4. **The payment record is shaped for spec 040 now.** `PaybackRequest` gets `paymentRunId String?`
