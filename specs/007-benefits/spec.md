...
- **Full-time rule:** no single (non-medical) benefit may exceed 50% of the pool; up to 4 picks (⇒ practically 2–4). **Part-time rule:** max 2 picks; budget = the part-time ceiling.
  > **Superseded by spec `012` (Company Coverage Rates, built 2026-08-07):** selection limits are now **FT 5 / PT 3**; the 50% cap and the pool total run on each benefit's **covered (company) share** (= cost × coverage rate), not the full amount. The employee enters the full cost; only the covered share draws from the pool. Part-time remains exempt from the 50% cap. See `specs/012-benefits-coverage/`.
...
- **FR-033**: In the submitted state the page MUST split into two **tabs** — "Your benefits" (the summary) and "Claims & reimbursement" — rather than one long scroll. The claims tab MUST present the claimable benefits as a **table**, one row per benefit, with columns: **#** (count), **Benefit** (name + guaranteed/basket · proof/note), **Allocated**, **Reimbursed**, **Pending**, **Left to claim**, and a per-benefit **Status** pill (Not started / Pending review / Partially reimbursed / Fully claimed / Rejected). Each row expands to reveal its **claim history** and the **file-a-new-claim** form (Proof: amount + note + mandatory proof upload; Request/Note: full-amount request + optional note). The table scrolls horizontally on narrow screens. (Superseded the earlier two-column card layout, 2026-08-03.)
- **FR-034**: While the employee scrolls either tab, the **tab bar MUST stay pinned** just beneath the sticky page header (the header's own compact-on-scroll behavior is unchanged), so switching tabs is always reachable.
- **FR-035**: Selection MUST be **prevented at the maximum**, not merely flagged: once the max count is reached (**4 full-time / 2 part-time**), every *unselected* benefit (including medical) is **dimmed and not selectable** until the employee deselects one — the UI never lets a 5th selection be made and then errored. The server keeps the count rule as a backstop.
- **FR-036**: When a submitted basket is **reopened**, any basket benefit with an **active claim** (status `PENDING` or `RELEASED`; rejected claims don't count) MUST be **locked**: it is dimmed, cannot be deselected, and its amount **cannot be reduced below the total already claimed** for it (it may still be raised). Medical follows the same rule. This is **enforced server-side** on save/submit (the client only mirrors it for UX) — a claimed benefit can never be removed or under-funded relative to its claims.
- **FR-037**: The employee-facing **"How the benefits basket works" guide** (`/benefits/policy`) MUST NOT expose confidential compensation config. It explains the rules in words, lists the available benefits by **name only**, and covers how to build a basket and how to claim/reimburse. It MUST NOT render the **pool-ceiling matrix** (amounts across all bands/types), **guaranteed-benefit amounts**, or the **medical rate card** — those live only on the admin Benefits page. An employee still sees their **own** pool ceiling and prices their **own** medical cover on the Benefits page itself; the guide never shows other bands' or other people's figures.
- **FR-038**: An employee MUST be able to **reopen their own submitted basket** without HR — a **"Update my basket"** action on the submitted view flips their selection back to `DRAFT` so they can commit the benefits they're sure of, claim against them, then return to allocate the rest and re-submit. The action is guarded: it affects **only the signed-in employee's own** selection, only while the **plan-year window is open**, and only when the selection is currently `SUBMITTED`. Already-filed claims are preserved, and the FR-036 locks apply on the re-save so nothing already claimed can be dropped or under-funded. HR's admin reopen remains for the window-closed / override case.
