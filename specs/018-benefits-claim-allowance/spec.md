...
2. **Given** the employee already has EGP 8,000 of covered claims recorded against one benefit and the pool ceiling is EGP 50,000, **When** they file another claim on the same benefit whose covered amount would take that benefit's cumulative covered total above EGP 25,000 (50% of pool), **Then** the claim is rejected with a clear message stating the remaining amount claimable on that benefit.
3. **Given** the employee's committed medical premium plus all covered flexible claims would exceed the pool ceiling, **When** they file a claim that crosses the ceiling, **Then** the claim is rejected with a "your pool is fully used — contact HR" message.
