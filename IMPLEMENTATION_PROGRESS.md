...
- [x] **Release sheet renamed "Release Guaranteed Benefit"** (page + entry button) and gains the **Super-User override**: SU sees every active employee (reason named for the unpriceable ones) and releases with a typed amount — validated server-side, SU-only, band figure always wins where it resolves, once-per-cycle claim guard still applies; HR Admin's sheet unchanged. 12/12 production checks.
