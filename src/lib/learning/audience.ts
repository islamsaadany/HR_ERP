...
/**
 * Audiences: routing rules compiled into ONE registry query (spec 038, research D2).
 *
 * WHY THIS SHAPE
 * A course is routed to "the Consulting department" or "everyone in their first two years" — and
 * that has to stay true as people join, move and leave, without anybody re-running anything
 * (FR-014, SC-003). So a `CourseAudience` row stores the RULE, never its expansion into member
 * rows, and this module turns a course's rules into a single `where` clause. One query answers
 * "who does this course reach" for the whole company; the same clause answers "does it reach this
 * person" with a userId added.
 *
