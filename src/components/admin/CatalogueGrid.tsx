  ...
  return (
    <div>
      ...
      <div className="mt-3 flex items-center justify-between">
        <p className="text-xs text-muted">{filtered.length} benefits · click a cell to edit · click a header to sort · drag a header to reorder</p>
        {err ? <p className="rounded bg-red-50 px-3 py-1 text-xs font-medium text-red-700">{err}</p> : null}
      </div>
      ...
      <div className="mt-3 ff-data-scroll rounded-xl border border-line bg-surface">
        <table className="ff-data-table text-sm">
          ...
          <tbody>
            ...
            {sorted.length === 0 ? (
              <tr><td colSpan={visibleCols.length} className="px-3 py-10 text-center text-sm text-muted">No benefits match this search.</td></tr>
            ) : null}
          </tbody>
        </table>
      </div>
