...
- **Benefits policy page** (`/benefits/policy`): a read-only **"How the benefits basket works"** explainer generated from the **live config** (rules, ceilings, guaranteed amounts, catalog, rate card, worked example) with **Print / Save-as-PDF**, linked from the employee Benefits page and admin. Always accurate — no static document to drift.
