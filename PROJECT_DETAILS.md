...
- **Cycle-length proration (spec `019`, revised 2026-08-11; migration `027`):** a plan year carries an admin-set **start/end window** (`PlanYear.startDate`/`endDate`, set via the **Plan year** dialog; end must be after start; blank = proration off). **Two distinct rules now apply** (the original release prorated everything only for mid-year *starters*; the pool/Prof-dev half was changed on 2026-08-11 to key off the **cycle length** so a short cycle scales the allowance for **everyone**):
  ...
  - **Medical → unchanged: mid-cycle-joiner proration only.** Medical unlocks at **3 months** and its **premium** is prorated by `annual × remaining whole months ÷ 12` measured **from the employee's own 3-month eligibility date** to the plan-year end (`classifyEligibility`/`prorate`); existing staff pay the **full** premium regardless of cycle length. A sub-6-month employee sees a **medical-only** Benefits view (basket + guaranteed shown as “unlocks at 6 months”), ceiling/rate looked up at the **entry 6mo–2y tier** (`poolCeilingFor(type, null)`).
  - **The CEILING is always the cycle's scale — the 3-month/6-month thresholds are doors, not scales (fixed 2026-08-23).** `poolCeiling`'s sub-6-month branch used to prorate the entry tier by the **mid-joiner** fraction, which is **1** whenever a person's 3-month mark falls on or before the cycle's first day — so on a six-month cycle a newcomer carried the **whole annual ceiling** (double their banded colleagues) with no *prorated* tag on the report to show it. Both branches now scale by the one `poolCycleFraction`; only the unlock threshold differs (3 months vs 6). The **premium** keeps its mid-joiner ÷12 reduction — that is where a mid-term joiner's reduction belongs, not on the container that bounds it. The same figure now drives the medical clamp on **both** commit paths (`commitMedical`, `manual-actions.loadMedicalContext` → `ceilingCap`), so the cap and the affordability refusal can no longer quote different ceilings.
...
- **Admin Benefits — management views (2026-08-16, mockups signed off):** the two working surfaces of `/admin/benefits` were rebuilt around scanning instead of reading. Both keep every figure, wording and action they had; nothing about the money rules or the spec-020 workflow changed.
  ...
  - **Claims** (`components/admin/ClaimsPanel.tsx`) — split into a **review queue** (only SUBMITTED claims: waiting time oldest-first, receipt, claim, a **"their pool after this"** meter, proof, Approve/Reject) and a **ledger** of everything decided (status/`Recorded by HR` filters, search, and a **footer total** of the filtered rows). The opened claim shows what the employee wrote, the 50%-cap working (unchanged wording), the reject-with-reason field, and a submit → HR → Finance **trail**. New **bulk approve** (`approveClaims` in `admin/benefits/actions.ts`) runs every selected claim through the same one-claim `approveOne` path — same status check, same Finance email each — and reports "Approved N of M" when someone else had already decided the rest. The pool meter is data the page derives per claimant (ceiling − medical drawn this cycle − non-rejected **flexible** claims); no schema change. **It counts only what the decision can move (fixed 2026-08-23):** it used to sum **every** claim in the cycle, so a 90,000 salary-driven **Loans** request read as pool spend and painted that employee's meter red at "EGP 0 left" on every row of theirs — nothing was ever mispaid (each write path asks `poolStateFor`, which counts `catalogItemId` claims alone), the screen simply said approving a loan would empty a benefits pool it cannot touch. A guaranteed claim now shows **"Not from the pool"** in that column instead of a meter (`QueueRow.poolFunded`), and the ceiling comes from **`poolCeiling`** rather than a local copy — the copy disagreed with the report for anyone under six months and never knew about a Super User's raised ceiling.
...
- **In-app cues, both directions**: the requester's decision badge (FR-014, `decisionSeenAt`, migration `016`) now shares the Time-Off nav badge with the **manager's pending-approvals count**, kept **live** by `TimeOffBadgeSync` polling `/api/time-off/badge` (mount + focus + 45s) and broadcasting `hrerp:timeoff-count` to `AppShell` — the same dead-badge fix pattern as data requests. ~~No email.~~ **Email since 2026-09-08 (spec 035 FR-013, CEO request):** submitting a request emails the approver(s) — `leaveApproversFor` in `src/lib/leave-queries.ts`, the inverse of `pendingApprovalWhere` (active manager, else every active Super User; one message each) — and approving/declining emails the requester (`leaveRequestedToApprover` / `leaveDecidedToEmployee` in `src/lib/email/templates.ts`, their own "Time off" eyebrow). Both fire after the write via `sendEmail` (fire-and-forget, env-gated, master toggle). Cancellations and HR deletions send nothing.
...
- **Editing the review tables in place (spec 009 · FR-006d, 2026-08-25, no migration):** the three sheets at the top of a cycle report are **editable on screen** — a wrong figure no longer means rebuilding a CSV and re-uploading. **Edit tables** turns People / Assignments / Contributions into a form (cells editable, rows addable + removable, contribution columns addable from the People table + removable) and a single **Recalculate** saves all three and re-renders the page, so every section below regenerates from the stored rows. Uploading is unchanged and still replaces a whole sheet. Design decisions: **Recalculate IS the save** (no second button, so the report can never show figures the DB doesn't hold), disabled until dirty, with a gold **Unsaved edits** chip on screen for exactly as long as the edits are browser-only (tracking *dirty*, not *saved*) and a confirming **Discard changes**; **every fault reported at once** in a `role="alert"` banner that is scrolled to + focused, with nothing written when any check fails; **status is chosen from a dropdown, not derived** from the closure-date column as the importer does (the operator is looking straight at it, so a closure date must not silently re-decide it); **a client short of 100% still saves** — flag-and-block is a payout rule, not a save rule — with the **Total** column re-adding live so a client is *seen* to reach 100% first; and **a rename in People follows the person** into Lead / BD / Lead source and their contributions column (on blur, not per keystroke). Code: `lib/incentive/review.ts` (payload model + `toDraft` + one pure `validateReview`, reusing the importer's now-exported `parseSheetNumber` so a figure typed into a cell and one pasted into a CSV can't mean two different things), `lib/incentive/persist.ts` (the write — deliberately **not** in the `"use server"` actions file, where every export becomes a browser-callable endpoint), `saveReviewTables` (access check → validate → write), `components/incentive/ReviewTables.tsx`, and `components/incentive/ReportSection.tsx` (section shell + table cell classes extracted from `CycleReport` so editor and report can't drift). The write is **replace-then-recreate** like an upload — the one statement order that can't trip the `(cycle, name)` / `(cycle, client)` unique indexes when two people trade names — carrying `eligibleToLead`/`utilization` across by row id so the two retired columns aren't silently dropped. **Recalculate is a plain button, not a form action, so its dispatch is wrapped in `startTransition`** — without it React never reports the pending state and the button neither says "Recalculating…" nor blocks a second click landing a second write (caught only by driving a real browser).
- **Dates in this module are `14-Jul 2026` (2026-08-25) — a deliberate exception to the platform's dd/mm/yyyy standard.** Requested so an entry can be checked at a glance: this is the one screen where somebody types compensation dates in off a spreadsheet, and a spelled month cannot be read the wrong way round. **`src/lib/incentive/dates.ts` both formats and parses**, so the displayed value, the typed cell, the saved value and a CSV cell can never disagree. Display is built from the date's **UTC** parts (a date-only value is stored at UTC midnight; reading it locally in a timezone behind UTC prints the day before). The editor's date cells are **typed text stating `dd-mmm yyyy`, deliberately not `<input type="date">`** — a native picker draws itself in the *browser's* UI language and, measured, rendered 1 March as `03/01/2021` under en-GB, ar-EG **and** en-US, so it cannot carry a house format. Cells accept the spelled month (short or full, any case) plus dd/mm/yyyy and ISO unstated, so nothing pasted in is needlessly refused; a made-up month, an unreal 31-Feb, or a bare number is refused by name (a bare number is an Excel serial only when it arrives off a **sheet**, never when typed). The CSV importer reads all-numeric cells **day-first**, which fixed a silent misread — `new Date("01/03/2021")` is American, so an operator's 1 March was being stored as 3 January with nothing to show for it (ISO cells and Excel serials unchanged; a legacy m/d/y sheet still reads correctly where the middle field can only be a day). Static and pre-filled **templates emit `14-Jul 2026`**, so download → fill → upload round-trips unchanged. The app-wide `formatDate`/`formatDateISO` (dd/mm/yyyy, `src/lib/labels.ts`) are unchanged and still govern every other module.
- **Business Partner Fee by person (2026-08-26):** a per-person table directly above **Commission by person**, same compact shape with a **click-a-name** breakdown. Computes nothing new — a person's fee is their lead fees plus contributor payments, i.e. the `Total` column below it — but derived **once** in `computeCycle` as `businessPartnerFeeByPerson` so the screen and any later export cannot drift. Each breakdown line is tagged **Lead** (client's GP%, the envelope it generated, the share the lead keeps) or **Contributor** (share, tier), because the two are earned by different rules; a contribution below the 5%-of-month floor shows as **0 (floored)** with its hover reason rather than vanishing. Both per-person tables share one expansion store keyed by table + name, so expanding somebody in one does not open them in the other. In the same change **Compensation by person** became **Total compensation by person**, and the shared `Section` header (`components/incentive/ReportSection.tsx`) puts the **title on its own line with the subtitle beneath** — previously title, subtitle and the action button competed for one line, so the subtitle was truncated and the longer the title the less survived. The `.xlsx` is unaffected (its sheets are `Summary` + one per person, not named from the section).
- **Releasing a cycle's payments into Finance (spec 009 FR-006g, 2026-08-26, migration `076`):** the business unit **head** — a new `BusinessUnitHead` appointment, built as a twin of `TransactionConfirmer` and making the same departure (holding Super User does **not** let you release; lock-out is prevented by self-appointment; no appointment means "all units") — ticks what they agree to pay on `/incentive/[id]/release`. Released lines become `IncentivePayout` rows and a **fourth `PayableKind`**, so they join the queue Finance already works from and end at the same per-unit bank confirmation; the CEO: *"the finance should see the release of amounts in the same place not to confuse him."* **Admin → Who moves money** (widened from "Who confirms transactions") shows all three steps per unit in the order money moves — *Releases payments* / *Creates the transaction* / *Confirms at the bank* — with the middle row read-only, because being Finance is an account role and not per unit, so it is shown there and set on the employee record. Deliberately not called "roles". **Matching is on Employee ID** (`lib/incentive/payouts.ts`, the one place that decides who a sheet name is): optional and deliberately non-unique, so a line releases only when the ID lands on exactly one active account, and the four other outcomes are refused **by name** — an ID matching somebody whose registry name differs from the sheet is releasable but held and shown. The People sheet, template and review table gain an **Employee ID** column. **Scheme fees and commission release separately**; `(cycle, user, kind)` is unique, so two people pressing at once cannot pay twice. **Amount and business unit are frozen at release.** **An edit that would change an already-released figure is refused, naming the person** (`released-guard.ts`) — worked from the released payments rather than the report's lines, because deleting a person produces no line at all and would otherwise sail through. The person is **emailed only when the confirmer marks the transaction complete** (fifth email workflow, third "your money is here" message; one email per person, not per payout), and its **words are editable** at Admin → Email notifications with a live preview — the amount rows are not, since they are the payment rather than wording; an unknown placeholder or a missing `{total}` is refused on save, and an untouched field is stored as NULL so it keeps tracking the built-in wording in code.
- **Proof:** `scripts/verify-incentive.ts` (Appendix A, 27/27), `scripts/verify-incentive-cycle.ts` (sample sheets, 23/23 — incl. the per-person fee reconciling to lead fees + contributor payments), `scripts/verify-incentive-release.mts` (releasing, 37/37 on a throwaway Postgres), and `scripts/verify-incentive-review-edit.mts` (in-place editing + dates, 71/71 on a throwaway Postgres: the stored→cells→stored round trip, the ±1pp tolerance, all seven faults at once, a 93% client still saving, `eligibleToLead`/`utilization` surviving a rename, removed rows going with their contributions, a name swap surviving the unique index, and the real `computeCycle` unblocking the corrected client). The screen itself was additionally driven end-to-end in a real browser.
...
- **A refused sign-in says which kind of refusal it was (2026-09-14).** `signIn()` throws an `AuthError` for two opposite situations, and the page used to print *"Incorrect email or password"* for both: `CredentialsSignin` (`authorize()` returned null — no such email, not `ACTIVE`, or the password doesn't verify) and **anything else**, in practice `CallbackRouteError` wrapping a Prisma `P1001` because the database is unreachable. Measured in a real browser with the **correct** password and Postgres stopped, the old code produced the byte-identical wrong-password sentence on the same `?error=Credentials` — so a company-wide outage was indistinguishable from a typo, which is how it was reported (*"all team is getting wrong password messages"*). Now: a real mismatch keeps its exact wording; everything else redirects to `?error=Unavailable`, whose message states the system can't be reached **and that a password reset won't help**, so nobody is sent to HR for an impossible fix. `err.type` + `err.cause` are `console.error`'d so the Vercel log names the cause, while the screen stays vague — it must never hint at whether an account exists. `authorize()` deliberately does **not** try/catch its Prisma reads: swallowing the throw would return null and turn the outage straight back into "wrong password".
...
optional parents, exactly one enforced by a check constraint — and, since migration `072`, two
possible LOCATIONS: an uploaded `blobUrl` or an `externalUrl` pointing at a receipt that lives
elsewhere, again exactly one) · `ExpenseSection` /
...
**Email.** One of the permitted workflows: `paybackSubmittedToFinance`, `paybackRejectedToEmployee`,
`paybackPaidToEmployee`. Fire-and-forget, master-toggleable. Petty cash sends none.

**The imported history (migrations `072` + `073`, 2026-08-25).** The marketing float did not start
empty: the whole MARCOM workbook was loaded — **12 periods, 144 lines, Oct 2024 → Aug 2026** —
generated by `scripts/import-marcom-workbook.mts`, which is kept as the reviewable statement of how
each row was derived. Every tab became one period, settled at its end by a real funding row (so the
arithmetic explains itself rather than being asserted by an opening balance nobody can trace);
**Aug 2026 is left OPEN owing 9,726.26**, the figure the CEO confirmed. Receipts are the workbook's
Google Drive links, stored as `externalUrl` — nothing was downloaded, so `/api/expense-evidence/[id]`
makes the same entitlement decision and **redirects** instead of streaming. Whoever follows the link
needs access to the file in Drive; the platform cannot grant that. Every tab's float total is
asserted against the total the sheet itself states, and the import against 9,726.26 — a mismatch on
