...
await db.department.deleteMany({ where: { name: { in: ["Consulting", "Empty Dept"] } } });

await db.department.createMany({ data: [{ name: "Consulting", order: 1 }, { name: "Empty Dept", order: 2 }] });
...
await mk("c1", { department: "Consulting", employmentType: "FULL_TIME" });
await mk("c2", { department: "Consulting", employmentType: "PART_TIME" });
await mk("c3", { department: "Consulting", employmentType: "PART_TIME" });
await mk("p1", { department: "Delivery", employmentType: "PART_TIME" });
...
const dept = await db.courseAudience.create({ data: { courseId: course.id, kind: "DEPARTMENT", value: "Consulting" } });
const empty = await db.courseAudience.create({ data: { courseId: course.id, kind: "DEPARTMENT", value: "Empty Dept" } });
...
check("part-time reaches its 3", counts.get(part.id), 3);
...
check("chips add up to 6", 3 + 0 + 3, 6);
check("but only 4 real people are reached", reached.length, 4);

console.log("\n── a rule with a value that no longer exists ──");
await db.department.deleteMany({ where: { name: "Consulting" } });
...
await db.department.create({ data: { name: "Consulting", order: 1 } });
...
const dupe = await db.courseAudience.count({
  where: { courseId: course.id, kind: "DEPARTMENT", value: "Consulting" },
});
